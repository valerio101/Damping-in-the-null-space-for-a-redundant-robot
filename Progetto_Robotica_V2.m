...
centro_x=0; %coordinata x del centro del cerchio
centro_y=5; %cordinata y del centro del cerchio
z0=0.5; %coordinata z del centro del cerchio
...
for i=1:10
    %traiettoria cercio 
    %pd=[centro_x+raggio*cos(omega);centro_y+raggio*sin(omega);z0 * ones(size(omega))];
    pd=[x;y;z];
    %dinamica del robot
    ...
    %formula tau
    Kp=1;
    a=pinv(J1)*(Kp*(pd(1:3,i)-f1)-Jdot1*qdot)-(eye(7,7)-pinv(J1)*J1)*0.01*g2;
    tau=M2*a+c2+g2;
...
figure;
plot3(pd(1,1:10), pd(2, 1:10), pd(3, 1:10), 'b', 'LineWidth', 2);
hold on 
plot3(array_fx(1:10), array_fy(1:10), array_fz(1:10), 'r', 'LineWidth', 2);
for i = 1:10
