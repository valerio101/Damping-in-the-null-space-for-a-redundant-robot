...
pd_cerchio = zeros(3, 100);
pd_cerchio_dot = zeros(3, 100);
pd_cerchio_ddot = zeros(3, 100);
for i=1:100
    pd_cerchio(1:3, i) = path(omega(i));
    pd_cerchio_dot(1:3, i) = path_dot(omega(i));
    pd_cerchio_ddot(1:3, i) = path_ddot(omega(i));
end

[r, M_num, c_num, g_num] = r.get_numerical_dyn_terms();
for i=1:100
    tic
    %dinamica del robot
    ...
    [r, A0e] = r.compute_direct_kinematics([array_q1(i); array_q2(i); array_q3(i); array_q4(i); array_q5(i); array_q6(i); array_q7(i)]);
    f1 = A0e(1:3, 4);
    [r, J1] = r.get_jacobian([array_q1(i); array_q2(i); array_q3(i); array_q4(i); array_q5(i); array_q6(i); array_q7(i)]);
    [r, Jdot1] = r.get_j_dot([array_q1(i); array_q2(i); array_q3(i); array_q4(i); array_q5(i); array_q6(i); array_q7(i);], [array_qdot1(i); array_qdot2(i); array_qdot3(i); array_qdot4(i); array_qdot5(i); array_qdot6(i); array_qdot7(i)]);
    ...
    % a=Kp*(pd_cerchio(1:3,i)-f1)+Ki*(pd_cerchio_dot(1:3,i)-J1*qdot) + pd_cerchio_ddot(1:3, i);
    % tau=Finv*(a-Jdot1*qdot+F*(c2+g2))+(eye(7,7)-Finv*F)*[1;1;1;1;1;1;1];
    % tau = double(tau);

    Jinv = pinv(double(J1));
    tau = double(J1' * Kp * (pd_cerchio(1:3,i)-f1) + Kv * (Jinv * pd_cerchio_dot(1:3,i) - qdot) + g2);
    ...
    [t, p]=ode45(@(t1,y)sistema(t1,y,tau,M2,c2,g2),[i i+1],[q;qdot]);
    % [t, p]=ode45(@(t1,y)sistema(t1,y,tau,M2,c2,g2),omega(1:i+1),[q;qdot]);
